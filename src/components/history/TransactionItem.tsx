...
export function TransactionItem({ txn, isLast }: Props) {
  ...
  name: { color: themeColors.textPrimary, fontSize: 13, fontWeight: '700' },
  subtitle: { color: themeColors.textSecondary, fontSize: 11, marginTop: 2 },
  time: { color: themeColors.textSecondary, fontSize: 10, marginTop: 2 },
  right: { alignItems: 'flex-end' },
  amount: { fontSize: 12.5, fontWeight: '700' },
  status: { color: themeColors.success, fontSize: 10, marginTop: 3 },
}),
    [themeColors]
  );
  ...
  return (
    <TouchableOpacity style={[styles.row, !isLast && styles.divider]}>
      ...
      <View style={styles.right}>
        <Text style={[styles.amount, { color: isIn ? themeColors.success : '#FF453A' }]}>
          {isIn ? '+ ' : '- '}
          {fmt(txn.amount)}
        </Text>
      ...
      <Feather name="chevron-right" size={15} color={themeColors.textSecondary} style={{ marginLeft: 6 }} />
    </TouchableOpacity>
  );
}
