...
type Action = { id: string; icon: keyof typeof Feather.glyphMap; title: string; sub: string };
  ...
  const { styles, iconSize } = useMemo(() => {
    ...
    return {
      ...
      styles: StyleSheet.create({
        ...
        title: { color: colors.textPrimary, fontSize: f(10.5), fontWeight: '700' },
        sub: { color: colors.textSecondary, fontSize: f(8.5), textAlign: 'center' },
      }),
    };
  }, [layoutScale]);
        ...
        return (
          ...
          >
            <View style={styles.iconBox}>
              <Feather name={a.icon} size={iconSize} color={colors.textPrimary} />
            </View>
  ...
  const { styles } = useMemo(() => {
    ...
    return {
      styles: StyleSheet.create({
        ...
        sectionTitle: { color: colors.textPrimary, fontSize: f(13.5), fontWeight: '700' },
        sectionSub: { color: colors.textSecondary, fontSize: f(10), marginTop: s(2) },
        viewMarkets: { flexDirection: 'row', alignItems: 'center', gap: s(2) },
        viewMarketsText: { color: colors.primaryLight, fontSize: f(10.5), fontWeight: '600' },
      }),
    };
  }, [layoutScale]);
    ...
    <View>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.sectionSub}>Buy, sell and trade crypto instantly</Text>
        </View>
        <TouchableOpacity style={styles.viewMarkets}>
          <Text style={styles.viewMarketsText}>View Markets</Text>
          <Feather name="chevron-right" size={12} color={colors.primaryLight} />
        </TouchableOpacity>
      </View>
