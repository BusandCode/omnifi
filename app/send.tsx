...
const BODY_ITEM_KEYS = ["recipient", "methods", "beneficiaries", "amount", "review"] as const;
...
const MOCK_BENEFICIARIES: Beneficiary[] = [
  { id: "1", initials: "IS", name: "Ibrahim S.", bank: "Kuda Bank", avatarColor: "#C4B5FD" },
  { id: "2", initials: "OA", name: "Oluwaseun A.", bank: "GTBank", avatarColor: "#FCA5A5" },
  { id: "3", initials: "MM", name: "Maryam M.", bank: "Access Bank", avatarColor: "#6EE7B7" },
  { id: "4", initials: "AS", name: "Aisha S.", bank: "Zenith Bank", avatarColor: "#93C5FD" },
];
...
export default function SendScreen() {
  ...
  if (currency === "USD" || currency === "EUR") {
    return <ForeignSendScreen currency={currency as ForeignCurrency} insets={insets} />;
  }
...
function NgnSendContent({ insets }: { insets: { top: number; bottom: number } }) {
  ...
  const [itemHeights, setItemHeights] = useState<Record<BodyItemKey, number>>({
    recipient: 0, methods: 0, beneficiaries: 0, amount: 0, review: 0,
  });
  ...
  useEffect(() => {
    const allMeasured =
      bodyContainerHeight > 0 && topHeight > 0 && BODY_ITEM_KEYS.every((k) => itemHeights[k] > 0);
    if (!allMeasured) return;

    const totalItemHeight = BODY_ITEM_KEYS.reduce((sum, k) => sum + itemHeights[k], 0);
  ...
  return (
    <View style={[styles.container, { paddingBottom: insets.bottom }]}>
      <View style={[styles.fixedHeader, { paddingTop: insets.top + 8 }]} onLayout={onTopLayout}>
        <SendHeader />
      </View>

      <ScaleProvider factor={factor}>
        <View
          style={[styles.content, { rowGap: gap, paddingBottom: BOTTOM_GAP, paddingTop: 4 }]}
          onLayout={onBodyContainerLayout}
        >
          <View style={styles.section} onLayout={makeItemLayoutHandler("recipient")}>
            <Text style={styles.sectionTitle}>Who are you sending to?</Text>
            <RecipientSearch value={recipient} onChangeText={setRecipient} />
          </View>

          <View style={styles.section} onLayout={makeItemLayoutHandler("methods")}>
            <SendMethodsGrid onSelect={handleMethodSelect} />
          </View>

          <View style={styles.section} onLayout={makeItemLayoutHandler("beneficiaries")}>
            <RecentBeneficiaries
          ...
          <View style={styles.section} onLayout={makeItemLayoutHandler("amount")}>
            <Text style={styles.sectionTitle}>Enter amount</Text>
            <AmountInput amount={amount} onChangeAmount={setAmount} note={note} onChangeNote={setNote} />
          </View>

          <View style={styles.section} onLayout={makeItemLayoutHandler("review")}>
            <ReviewSummary amount={numericAmount} fee={0} onSubmit={handleSubmit} loading={loading} />
          </View>
        </View>
      </ScaleProvider>
    </View>
...
const styles = StyleSheet.create({
  ...
  fixedHeader: { paddingHorizontal: 16, paddingBottom: 4, backgroundColor: colors.background },
  content: { flex: 1, paddingHorizontal: 16 },
  foreignScrollContent: { paddingHorizontal: 16, paddingTop: 8, paddingBottom: 24 },
  section: { gap: 8 },
  sectionTitle: { fontSize: 11.5, fontWeight: "700", color: colors.textPrimary },
});